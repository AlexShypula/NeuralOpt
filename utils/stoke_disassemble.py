...
def collect_and_disas(path_to_bin_dir: str, path_disas_dir: str, optimization_flag: str,
    ...
    assert not os.path.exists(successful_path_out_file), "the out-file specified already exists, please change or delete"
    ...
    successful_paths = parallel_disassemble(binary_info_dict=binary_info_dict, path_to_bin_dir=path_to_bin_dir,
                                            path_to_disas_dir=path_disas_dir, optimization_flag=optimization_flag,
...
def parallel_disassemble(binary_info_dict: Dict[str, Dict], path_to_bin_dir: str,  path_to_disas_dir: str,
    ...
    for binary_identifier, binary_dictionary in tqdm(binary_info_dict.items()):
        ...
        if file_hash not in running_hash_set:
            ...
            rel_path_to_bin_identifier = collapse_path(rel_path_to_bin_identifier)
    ...
    with tqdm(total=len(jobs_list), smoothing=0) as pbar:
        if debug:
            for bin_pth, msg, rc in map(copy_and_disas_wrapper, jobs_list):
                pbar.update()
...
def copy_and_disas(path_to_disas_dir: str, path_to_bin_dir: str, rel_path_to_bin_identifier: str,
	...
	try:
		lcl_bin_fldr = os.path.join(path_to_disas_dir, rel_path_to_bin_identifier, optimization_prefix, "bin")
		os.makedirs(lcl_bin_fldr)

		lcl_fun_fldr = os.path.join(path_to_disas_dir, rel_path_to_bin_identifier, optimization_prefix, "functions")
	...
	path_to_orig_bin = os.path.join(path_to_bin_dir, binary_dictionary["repo_path"], binary_dictionary["file_hash"])
